...
NIGHTJAR_MAX_REGRESSION="${DS4_DSPARK_NIGHTJAR_MAX_PREDICTED_REGRESSION:-0.03}"
...
if [[ -n "${DS4_DSPARK_SPS_PROFILE+x}" ]]; then
  ...
  SPS_PROFILE_DEFAULT="$MODEL_DIR/dspark-sps-${DSPARK_VARIANT}.conf"
  SPS_PROFILE_REPO="$(cd "$(dirname "$0")" && pwd)/profiles/dspark-sps-${DSPARK_VARIANT}.conf"
  SPS_PROFILE="$SPS_PROFILE_DEFAULT"
  if [[ ! -f "$SPS_PROFILE_DEFAULT" && -f "$SPS_PROFILE_REPO" ]]; then
    SPS_PROFILE="$SPS_PROFILE_REPO"
  fi
fi
...
export DS4_DSPARK_NIGHTJAR_MAX_PREDICTED_REGRESSION="$NIGHTJAR_MAX_REGRESSION"
...
echo "DSpark Nightjar: active=$NIGHTJAR shadow=$NIGHTJAR_SHADOW aggregate-budget=1..R*$DRAFT (B=0 singleton-only) t-2-capacity current-confidence-ranked executor=exact-shape-aware recent-reward=pure-neural-ms/token max-predicted-regression=$NIGHTJAR_MAX_REGRESSION persistent-warm-start=bounded"
